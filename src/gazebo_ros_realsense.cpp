...
namespace gazebo
{
  ...
  void GazeboRosRealsense::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    ROS_INFO("Realsense Gazebo ROS plugin loading for model %s.", _model->GetName().c_str());
                                      ...
                                      const transport::PublisherPtr pub)
  {
    ...
    image_msg_.header.stamp.sec = current_time.sec;
    image_msg_.header.stamp.nsec = current_time.nsec;
    ...
    fillImage(image_msg_, pixel_format, cam->ImageHeight(),
    ...
    auto camera_info_msg =
        cameraInfo(image_msg_, cameras.at(camera_id)->HFOV().Radian());
    ...
    for (uint32_t j = 0; j < rows_arg; j++)
    {
      ...
      for (uint32_t i = 0; i < cols_arg; i++, ++iter_x, ++iter_y, ++iter_z, ++iter_rgb)
      {
        ...
        if (real_depth > pointCloudCutOff_ && real_depth < pointCloudCutOffMax_)
        {
          ...
          // in order to ressolve this change the coorindates towards [y, -z, x]
          *iter_y = depth * tan(yAngle);
          *iter_z = -1.0 * depth * tan(pAngle);
          *iter_x = depth;
        }
        ...
        uint8_t *image_src = (uint8_t *)(&(this->image_rgb_msg_.data[0]));
        if (this->image_rgb_msg_.data.size() == rows_arg * cols_arg * 3)
        {
          // color
          if (this->image_rgb_msg_.encoding == sensor_msgs::image_encodings::RGB8)
          {
            iter_rgb[2] = image_src[i * 3 + j * cols_arg * 3 + 0];
            iter_rgb[1] = image_src[i * 3 + j * cols_arg * 3 + 1];
            iter_rgb[0] = image_src[i * 3 + j * cols_arg * 3 + 2];
          }
          else if (this->image_rgb_msg_.encoding == sensor_msgs::image_encodings::BGR8)
          {
          ...
          {
            throw std::runtime_error("unsupported colour encoding: " + this->image_rgb_msg_.encoding);
          }
        }
        else if (this->image_rgb_msg_.data.size() == rows_arg * cols_arg)
        {
